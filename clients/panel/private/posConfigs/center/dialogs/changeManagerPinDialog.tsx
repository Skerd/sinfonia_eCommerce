...
type Props = WithLanguageType &
    WithAxiosType<ActionMessage & {hasManagerPin: true}, PostPayload> & {
        open: boolean;
        onClose: () => void;
        config: PosConfig;
...
function ChangeManagerPinDialog({
    open,
    onClose,
    config,
    onSuccess,
    ...
    useImperativeHandle(innerRef, () => ({
        success: () => {
            onSuccess?.({hasManagerPin: true, currentUserHasManagerPin: true});
            onClose();
        },
    }));
    ...
    const submit = () => {
        ...
        if (!trimmedCurrent || !trimmed || !trimmedConfirm || loading) return;
        if (trimmed !== trimmedConfirm) {
            toast.error(String(resolveLanguageKey("pinMismatch")));
            return;
        }
        if (trimmed.length < 4) {
            toast.error(String(resolveLanguageKey("pinTooShort")));
            return;
        }
        onFilterChange({
            _id: config._id,
        ...
        <Dialog open={open} onOpenChange={(next) => !loading && !next && onClose()}>
            <DialogContent className="sm:max-w-sm" onOpenAutoFocus={(e) => e.preventDefault()}>
                ...
                <DialogFooter className="gap-2 px-4 py-4 sm:gap-2">
                    ...
                    <Button
                        type="button"
                        disabled={!currentPin.trim() || !pin.trim() || !confirmPin.trim() || loading}
                        onClick={submit}
                    >
...
export default compose(
    withLanguage(
        "src/modules/eCommerce/clients/panel/private/posConfigs/center/dialogs/changeManagerPinDialog.tsx",
    ),
    withAxios<ActionMessage & {hasManagerPin: true}, PostPayload>(
        {url: "/api/eCommerce/posConfig/changeManagerPin", method: "POST", data: {}},
        true,
    ),
    withDebug(true, true),
